...
class JsonImporter(outputFile: String) extends Importer {
  ...
  private val bz2Os = new BZip2CompressorOutputStream(
    new BufferedOutputStream(new FileOutputStream(outputFile), 32768 * 100))
...
class CassandraImporter(clusterHost: String) extends Importer {
  ...
  private val statement = session.prepare(
  ...
  private def connect(node: String): Cluster = {
